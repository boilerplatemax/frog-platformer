...
namespace TarodevController
{
    ...
    public class PlayerController : MonoBehaviour, IPlayerController, IPhysicsObject
    {
        ...
        [SerializeField] private int maxGrapples = 1; // Add this line
        ...
        [SerializeField] public bool autoAim = true;
        ...
        public int WallDirectionForJump { get; private set; }
...
private void CalculateDeathVelocity(){
    if(!_alive){
       ...
       private void ToggleGrounded(bool grounded)
        {
            ...
            if (grounded)
            {
                ...
                currentGrapples = 0; // Reset the grapple count when grounded
            }
            else
            {
        ...
        private void ExecuteJump(JumpType jumpType)
        {
            ...
            else if (jumpType is JumpType.WallJump)
            {
                ...
                if (_isOnWall || IsPushingAgainstWall)
                {
                    ...
                    WallDirectionForJump=_wallDirectionForJump;
                    //Debug.Log(_wallDirectionForJump);
                }
                else
                {
        ...
        #region PullUp
private Vector2 _pullUpStartPos;
private Vector2 _pullUpEndPos;
private float _pullUpTimer = 0f;
public float pullUpDuration = 1f; // Public value for the duration of the pull-up

private void CalculatePullUp()
{
    if (isPullingUp)
    {
        _rb.gravityScale = 0;
        _pullUpTimer += Time.deltaTime;
        float progress = _pullUpTimer / pullUpDuration;
        transform.position = Vector2.Lerp(_pullUpStartPos, _pullUpEndPos, progress);

        if (progress >= 1f)
        {
            isPullingUp = false;
            // Additional logic after the pull-up completes (e.g., resetting state, triggering animations)
        }
    }
}

private void StartPullingUp(Vector2 targetPosition)
{
    Debug.Log("pull up end pos: "+ _pullUpEndPos);
    isPullingUp = true;
    _pullUpStartPos = transform.position;
    _pullUpEndPos = new Vector2(transform.position.x, transform.position.y + 1.5f); // Set the end position 1 unit above the start position
    _pullUpTimer = 0f;
    // Trigger pull-up animation here
}
    ...
    private void CalculateDash()
    {
        ...
        if (_dashToConsume && _canDash && !Crouching && _time > _nextDashTime)
        {
            ...
            //makes it so player can't dash into a wall
            if(_isOnWall&&_frameInput.Move.y!=1){
                return;
            }
        ...
        if (_dashing)
        {
            if (_time > _startedDashing + Stats.DashDuration)
            {
                ...
                SetVelocity(new Vector2(Velocity.x * Stats.DashEndHorizontalMultiplier, Velocity.y* Stats.DashEndHorizontalMultiplier));
...
private void StartGrapple()
{
    ...
    if (autoAim)
    {
        targetPoint = FindNearestGrapplePoint();
    }
    else
    {
    ...
    if (hit.collider != null)
    {
        ...
        Debug.Log($"Grapple hit: {hit.collider.name} at position: {hit.point}");
        isGrappling = true;
        grapplePoint = hit.point;

        Vector2 grappleDirection = (grapplePoint - (Vector2)transform.position).normalized;
        _rb.AddForce(grappleDirection * moveToGrappleSpeed, ForceMode2D.Impulse);

        springJoint.enabled = true;
        springJoint.connectedAnchor = grapplePoint;
        springJoint.distance = 0;
        springJoint.dampingRatio = springDamp;
    ...
    {
        Debug.Log("Grapple missed - no hit detected");
    }
}

private Vector2 FindNearestGrapplePoint()
{
    Collider2D[] colliders = Physics2D.OverlapCircleAll(transform.position, autoAimRadius, grappleLayer);
...
private void UpdateGrappleLine()
{
    ...
    if (grappledObject != null && grappledObject.CompareTag("HookFly"))
    {
        ...
        {
            Bounds bounds = grappledObject.GetComponent<Collider2D>().bounds;
            grapplePoint = new Vector2(bounds.center.x + grappleHandler.grappleOffset.x, bounds.min.y + grappleHandler.grappleOffset.y);
            springJoint.connectedAnchor = grapplePoint;
        }
    }

    float maxEffectDuration = 0.2f;
    float effectFactor = Mathf.Clamp01(1 - moveTime / maxEffectDuration);

    for (int i = 0; i < lineRenderer.positionCount; i++)
    {
        float t = (float)i / (lineRenderer.positionCount - 1);
        Vector2 linePosition = Vector2.Lerp(firepoint.position, grapplePoint, t);

        if (t > 0 && t < 1)
        {
            float currentWaveAmplitude = waveAmplitude * effectFactor * Mathf.Sin(Mathf.PI * t);
            float currentWaveFrequency = waveFrequency * effectFactor;
            float currentNoiseScale = noiseScale * effectFactor;

            float waveOffset = Mathf.Sin(t * currentWaveFrequency + moveTime * currentWaveFrequency) * currentWaveAmplitude;

            float noise = Mathf.PerlinNoise(t * currentNoiseScale, moveTime * currentNoiseScale) * currentWaveAmplitude;
            waveOffset *= noise;

            Vector2 perpendicular = Vector2.Perpendicular(grapplePoint - (Vector2)firepoint.position).normalized;
            linePosition += perpendicular * waveOffset;
        }
    ...
    if(!slowTimeOnGrapple)return;
     // Determine the target time scale based on the grappling state
        ...
        private void OnTriggerEnter2D(Collider2D other)
        {
            ...
            if (other.CompareTag("ClimbableTop")){
                Debug.Log("Colliding with climbable top");
            }
            // Check if the player reaches the top of the climbable object
            if (other.CompareTag("ClimbableTop")&&_isOnWall&&!_grounded)
            {
                

                // Start pulling up to the top position of the climbable object
                StartPullingUp(other.transform.position);
            }
        }
