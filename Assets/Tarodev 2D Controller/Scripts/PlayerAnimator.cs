...
namespace TarodevController
{
    public class PlayerAnimator : MonoBehaviour
    {
        ...
        [SerializeField] private AudioClip[] _grappleClips;
        ...
        private bool _grappleSoundPlayed=false;
...
private void CheckIfFalling()
        ...
        private void HandleWallSlideEffects()
        {
            ...
            if ((_player.ClimbingLadder || _isOnWall) && _player.Velocity.y > 0)
            {
            ...
            else
                {
                    ...
                    // Climbing animation will play when we are sliding down
                    if (_isOnWall||_player.ClimbingLadder )
                    {
        ...
        #region Grapple
        private void HandleGrappleDetection()
{
    if (_player.isGrappling)
    {
        _anim.SetBool("Grappling", true);

        if (!_grappleSoundPlayed)
        {
            PlayRandomSound(_grappleClips, 0.2f, Random.Range(0.98f, 1.02f));
            _grappleSoundPlayed = true;
        }
    }
    else
    {
        _anim.SetBool("Grappling", false);
        _grappleSoundPlayed = false;
    }
}

        private void HandleGrappleFlip()
{
    if (_player.grapplePoint.x > transform.position.x)
    {
        _sprite.flipX = false;
    }
    else
    {
        ...
        {
            // Assuming _player is your player object with a Velocity property
            if (Mathf.Abs(_player.Velocity.x) < 0.1f && Mathf.Abs(_player.Velocity.y) < 0.1f)
            {
                // Player is considered idle
                _anim.SetBool("isIdle", true);  // Replace "isIdle" with the actual parameter name in your Animator
            }
            else
            {
                // Player is not idle
                _anim.SetBool("isIdle", false);  // Replace "isIdle" with the actual parameter name in your Animator
            }
        }

        private void HandlePullUpDetection(){
            if(_player.isPullingUp){
                
                _anim.SetBool("isPullingUp", true);
            }
            else{
                _anim.SetBool("isPullingUp", false);
            }
        }
        ...
        private void OnJumped(JumpType type)
        {
            if (type is JumpType.Jump or JumpType.Coyote or JumpType.WallJump)
            {
                ...
                {
                if (_player.WallDirectionForJump == 1)
                {
                    _sprite.flipX = true;
         
                    //Debug.Log(_player.WallDirection);
                }
                else
                {
                    _sprite.flipX = false;
        ...
        private void OnGroundedChanged(bool grounded, float impact)
{
    ...
    if (grounded)
    {
        ...
        _anim.SetTrigger("Land"); // Set the Land trigger when grounded
        CancelSquish();
        ...
        private void SetParticleColor(Vector2 detectionDir, ParticleSystem system)
{
    ...
    _currentGradient = Color.white;
    
    if (ray.transform.TryGetComponent(out SpriteRenderer r))
    {
        _currentGradient = new ParticleSystem.MinMaxGradient(r.color * 0.9f, r.color * 1.2f);
    }
    else if (ray.transform.TryGetComponent(out Tilemap tilemap))
    {
        _currentGradient = new ParticleSystem.MinMaxGradient(tilemap.color * 0.9f, tilemap.color * 1.2f);
    }
...
private void SetColor(ParticleSystem ps)
{
    ...
    main.startColor = _currentGradient;
        ...
        private static readonly int AirJump = Animator.StringToHash("AirJump");
        private static readonly int DashKey = Animator.StringToHash("Dashing");
