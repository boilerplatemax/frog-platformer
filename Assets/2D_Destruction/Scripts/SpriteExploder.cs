    ...
    private static GameObject generateVoronoiPiece(GameObject source, List<Vector2> region, Vector2 origVelocity, Vector3 origScale, Quaternion origRotation, Material mat)
{
    ...
    Mesh uMesh = new Mesh();
    uMesh.vertices = calcVerts(new Voronoi(region, null, getRect(region)));
    uMesh.triangles = calcTriangles(new Voronoi(region, null, getRect(region)));

    if (source.GetComponent<SpriteRenderer>() != null)
    {
        uMesh.uv = calcUV(uMesh.vertices, source.GetComponent<SpriteRenderer>(), source.transform);
    }
    else
    {
        uMesh.uv = calcUV(uMesh.vertices, source.GetComponent<MeshRenderer>(), source.transform);
    }
    ...
    // Set layer by name
    piece.layer = LayerMask.NameToLayer("FragmentEffect");

    // Attach FadeAndDestroy script
    FadeAndDestroy fadeScript = piece.AddComponent<FadeAndDestroy>();
